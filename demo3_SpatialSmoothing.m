...
imgFin = uint8(zeros(size(imgFull)));
...
for k = 1:3
    for i = 1:length(imgFull(:,1))
       for j = 1:length(imgFull)
           imgFin(i,j,k) = round((0.1111*imFil(i,j,k))+(0.1111*imFil(i,j+1,k))+(0.1111*imFil(i,j+2,k))+(0.1111*imFil(i+1,j,k))+(0.1111*imFil(i+1,j+1,k))+(0.1111*imFil(i+1,j+2,k))+(0.1111*imFil(i+2,j,k))+(0.1111*imFil(i+2,j+1,k))+(0.1111*imFil(i+2,j+2,k)));
       end
    end
end
...
subplot(1,2,2);
imshow(imgFin);
title("Low Pass / spatial smoothing");
